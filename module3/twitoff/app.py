...
def create_app():
    ...
    def compare():
        ...
        if user0 == user1:
            ...
            if prediction:
                message = f'"{hypo_tweet_text}" is more likely something {user1} would tweet, not {user0}'
            else:
                message = f'"{hypo_tweet_text}" is more likely something {user0} would tweet, not {user1}'
